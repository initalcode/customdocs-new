...
public class ProviderDao extends JdbcDaoSupport {
	...
	public void insertFacility(Facility facility) {
		String sql = "insert into facility (facilityname, street, town, state, zip, phone, npi, taxid, fax, billingPhone) values (?,?,?,?,?,?,?,?,?,?)";
		this.getJdbcTemplate().update(sql, new Object[]{facility.getFacilityName(), facility.getStreet(), facility.getTown(),
				facility.getState(), facility.getZip(), facility.getPhone(), facility.getNpi(), facility.getTaxId(), facility.getFax(), facility.getBillingPhone()});
	}
	...
	public void updateFacility(Facility fac){
		String sql = "update facility set facilityname=?, street=?, town=?, state=?, zip=?, phone=?, npi=?, taxid=?, fax=?, billingPhone=? where id=?";
		this.getJdbcTemplate().update(sql, new Object[]{fac.getFacilityName(), fac.getStreet(), fac.getTown(), fac.getState(),
							fac.getZip(),fac.getPhone(), fac.getNpi(), fac.getTaxId(), fac.getFax(), fac.getBillingPhone(), fac.getId()});
	}
	...
	private static final class FacilityMapper implements RowMapper<Facility> {
	    ...
	    public Facility mapRow(ResultSet rs, int rowNum) throws SQLException {
	        ...
	        facility.setBillingPhone(rs.getString("billingPhone"));
	        return facility;
	    }
	}
