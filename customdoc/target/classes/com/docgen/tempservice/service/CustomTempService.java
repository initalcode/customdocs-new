...
public class CustomTempService {
	...
	@Autowired
	com.docgen.tempservice.dao.PCPLetterDao pcpDao;
	...
	public List<PCPLetterDetails> getPCPLetters(){
		return pcpDao.getPCPLetters();
	}
	@DELETE
	@Path("/pcpletters")
	public void removePCPLetter(@QueryParam("pcpId") long pcpId){
		pcpDao.removePCPLetter(pcpId);
	}
	@POST
	@Path("/genpcpletter")
	@Produces("application/octet-stream")
	public Response generatePCP(PCPLetterDetails pcp) throws TemplateException {
		pcpDao.savePCPDetails(pcp);
		Patient currentPatient = patientDao.getPatientForId(pcp.getPatientId());
		Facility currentFacility = providerDao.getFacilityForId(pcp.getFacilityId());
		DoctorDto currentDoctor = providerDao.getDoctorForId(pcp.getDoctorId());
		
		Map<String, Object> data = new HashMap();
		data.put("patient", currentPatient);
		data.put("facility", currentFacility);
		data.put("doctor", currentDoctor);
		data.put("dateCreated", pcp.getDateCreated());
		data.put("amountDue", pcp.getAmountDue());
		data.put("dosNotPaid", pcp.getDosNotPaid());
		data.put("startDate", pcp.getStartDate());
		
		File returnFile = fi.process("pcptemplate.html", data, "pcpletter.html");
		ResponseBuilder resp = Response.ok((Object) returnFile);
		resp.header("Content-Type", "application/octet-stream");
		resp.header("Content-Disposition", "attachment; filename=\"" + "pcpletter" + "\"");
		return resp.build();
